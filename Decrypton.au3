	  ...
	  $List = _FileListToArrayRec($Drives[$PTR_1],"*",$FLTAR_FILES+$FLTAR_NOSYSTEM,$FLTAR_RECUR,$FLTAR_FULLPATH)
	    ...
	    For $PTR_2 = 1 To $List[0] Step 1
			...
			If StringLeft(StringReverse($TempPath[1]),2) = "$_" Then
			   ...
			   Next
			   $FileExt = StringSplit(StringReverse($TempPath[1]),"_")[2]
			   $FilePath = StringReverse($FilePath)
			   $FileNameNew = $FilePath & StringTrimLeft(StringReverse($TempPath[1]),StringLen($FileExt)+3) & "." & $FileExt
			   ;MsgBox("","Display",$FileNameNew)
			   _Crypt_DecryptFile($FileName,$FileNameNew,"YouCantTouchMeEvenThoughYouCan",$CALG_3DES)
			   FileDelete($FileName)
			EndIf
